...
# Benchmark Results (audio/2026/02/260106_0614.mp3)

## Command

```bash
make benchmark AUDIO=audio/2026/02/260106_0614.mp3 MODEL_NAME=base.en RUN_WHISPERX=1 RUN_WHISPERX_DIARIZE=1
```
...
| Engine | Real | User | Sys | Result |
| --- | --- | --- | --- | --- |
| whisper.cpp | 12.06 s | 40.74 s | 0.93 s | Success; transcript at `/tmp/stt-bench/whispercpp.txt`. |
| faster-whisper | 6.39 s | 11.82 s | 2.17 s | Success; transcript at `/tmp/stt-bench/faster-whisper.txt`. |
| parakeet | 9.84 s | 21.51 s | 3.06 s | Success; transcript at `/tmp/stt-bench/parakeet.txt`. |
| whisperx-fp16 | 13.36 s | 7.59 s | 5.15 s | Failed: CPU/backend does not support float16 compute. |
| whisperx-fp32 | 71.74 s | 162.10 s | 39.55 s | Completed with warnings (Pyannote/Torch version mismatch, torch audio backend deprecations). Transcript at `/tmp/stt-bench/whisperx-fp32.txt`. |
| whisperx-diarize | 167.28 s | 519.32 s | 71.01 s | Completed with warnings (Pyannote/Torch version mismatch, torch audio backend deprecations). Transcript at `/tmp/stt-bench/whisperx-diarize.txt`. |

All transcripts (successful runs or placeholder files) live under `/tmp/stt-bench`.
