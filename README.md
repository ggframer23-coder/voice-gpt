...
- Store transcripts in a local journal (SQLite + FAISS or ChromaDB index).
...
```bash
stt init
stt audio /path/to/audio.wav base.en
```
...
```bash
make audio AUDIO=audio MODEL=base.en
```
...
- `stt audio [AUDIO] [MODEL]` - transcribe a file or directory (defaults to `audio/` recursively).
...
- `stt list` - list ingested audio with size, minutes, and word count.
...
- `--recursive` / `-r` (ingest-dir) to include subdirectories
...
- Run `stt audio --engine whisperx --whisperx-diarize` to include speaker labeling (metadata is stored under `metadata["whisperx"]["diarization"]`).
...
- WhisperX diarization uses gated Pyannote models. Set `HUGGINGFACE_TOKEN` (or `HF_TOKEN`/`HUGGINGFACE_HUB_TOKEN`) and accept model terms for `pyannote/speaker-diarization` and `pyannote/segmentation` on Hugging Face. Download once online, then offline runs will use the cache.
...
make audio AUDIO=/path/to/audio.wav MODEL=/path/to/model.gguf
...
- `STT_INDEX_BACKEND` to choose `faiss` (default) or `chroma`
- `STT_HOME` to change the base directory (defaults to `~/.stt` for FAISS, `~/.stt-chroma` for ChromaDB)
- `STT_DB` to point directly at a specific SQLite file

The vector index lives alongside the database as `~/.stt/faiss.index` or `~/.stt-chroma/chroma`
unless you set `STT_INDEX`.
...
```bash
stt audio /path/to/audio.wav /path/to/gguf-model --no-convert
```

Split speech into VAD clips and include timestamps:

```bash
stt audio /path/to/audio.wav /path/to/gguf-model --vad
```
...
- FAISS index + SQLite live in `~/.stt` by default; ChromaDB uses `~/.stt-chroma`.
- For a different location, set `STT_INDEX_BACKEND` and/or `STT_HOME` or `STT_DB`/`STT_INDEX`.
