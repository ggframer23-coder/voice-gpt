.DEFAULT_GOAL := help
.PHONY: help clean venv install install-cli install-faster install-cpu install-cli-cpu install-faster-cpu install-whisper download-model benchmark benchmark-parakeet init audio reingest ingest-dir query add-text dedupe test guard-stt guard-whisper-bin guard-whisper-src guard-model-file
...
REINGEST_FLAGS :=
ifeq ($(NO_CONVERT),1)
REINGEST_FLAGS += --no-convert
endif
ifeq ($(NO_INGEST),1)
REINGEST_FLAGS += --no-ingest
endif
ifneq ($(SAVE),)
REINGEST_FLAGS += --save $(SAVE)
endif
...
help:
	...
	@ echo "  audio               Transcribe AUDIO with MODEL"
	...
	@ echo "  make audio AUDIO=/path/a.wav MODEL=/path/model.gguf"
	@ echo "  make audio AUDIO=/path/a.wav MODEL=base.en ENGINE=faster-whisper"
	...
	@ echo "  make reingest AUDIO=/path/audio"
	@ echo "  make reingest -r AUDIO=/path/audio"
...
audio: guard-stt guard-whisper-bin guard-model-file guard-AUDIO guard-MODEL
	@ if [ -d "$(AUDIO)" ]; then \
...
reingest: ENGINE=auto
reingest: guard-stt guard-AUDIO
	$(STT) reingest $(if $(REINGEST_RECURSIVE),-r,) "$(AUDIO)" $(MODEL_NAME) $(REINGEST_FLAGS) --engine auto
